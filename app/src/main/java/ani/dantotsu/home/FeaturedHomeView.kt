...
class FeaturedHomeView @JvmOverloads constructor(context: Context, attrs: android.util.AttributeSet? = null) : FrameLayout(context, attrs) {
    ...
    init {
        ...
        val card = MaterialCardView(context).apply { radius = dp(24).toFloat(); cardElevation = dp(2).toFloat(); strokeWidth = 0 }
        val contentFrame = FrameLayout(context)
        image.scaleType = ImageView.ScaleType.CENTER_CROP
        contentFrame.addView(image, FrameLayout.LayoutParams(-1, -1))
        contentFrame.addView(View(context).apply { background = GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, intArrayOf(0x12000000, 0xB8000000.toInt(), 0xF0000000.toInt())) }, FrameLayout.LayoutParams(-1, -1))
        val content = LinearLayout(context).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.BOTTOM; setPadding(dp(18), dp(18), dp(18), dp(18)) }
        badge.apply { text = "FEATURED"; textSize = 10f; setTextColor(Color.WHITE); typeface = ResourcesCompat.getFont(resources, R.font.poppins_bold); setPadding(dp(9), dp(4), dp(9), dp(4)); background = GradientDrawable().apply { cornerRadius = dp(8).toFloat(); setColor(0xAA000000.toInt()) } }
        content.addView(badge, LinearLayout.LayoutParams(-2, -2).apply { bottomMargin = dp(8) })
        title.apply { textSize = 21f; maxLines = 2; setTextColor(Color.WHITE); typeface = ResourcesCompat.getFont(resources, R.font.poppins_bold) }
        content.addView(title, LinearLayout.LayoutParams(-1, -2))
        description.apply { textSize = 11.5f; maxLines = 2; setTextColor(0xE6FFFFFF.toInt()); typeface = ResourcesCompat.getFont(resources, R.font.poppins) }
        content.addView(description, LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(4) })
        action.apply { text = "View Anime"; textSize = 11f; minHeight = dp(38); setPadding(dp(14), 0, dp(14), 0); cornerRadius = dp(14) }
        ...
        card.addView(contentFrame, FrameLayout.LayoutParams(-1, -1))
        addView(card, LayoutParams(-1, dp(300)))
        post { bindModel() }
    }
