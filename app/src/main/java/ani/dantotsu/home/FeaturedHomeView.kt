...
class FeaturedHomeView @JvmOverloads constructor(
    ...
    init {
        ...
        cardContent.addView(image, FrameLayout.LayoutParams(-1, -1))

        val gradient = View(context).apply {
            background = GradientDrawable(
                GradientDrawable.Orientation.TOP_BOTTOM,
                intArrayOf(0x12000000, 0xB8000000, 0xF0000000)
            )
        }
        cardContent.addView(gradient, FrameLayout.LayoutParams(-1, -1))
        ...
        cardContent.addView(content, FrameLayout.LayoutParams(-1, -1))
    ...
    private fun bindModel() {
        ...
        model.getRecommendation().observe(owner) { list ->
            val media = list?.firstOrNull() ?: return@observe
            render(media)
        }
    }
    ...
    private fun render(media: Media) {
        ...
        val rawDescription = media.description?.let { Html.fromHtml(it, Html.FROM_HTML_MODE_LEGACY).toString().trim() }
        description.text = rawDescription?.takeIf { it.isNotBlank() } ?: "Discover this anime on KakaAnime."
